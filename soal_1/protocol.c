...
int username_exist(char *nama) {
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (strcmp(username[i], nama) == 0)
            return 1; // [FIX] tambah return 1 jika ditemukan
    }
    return 0;
}
...
int main() {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) { // [FIX] i = 0 bukan i = 9
        client_sockets[i] = 0;
        strcpy(username[i], "");
    }
    ...
    while (1) {
        ...
        int max_sd = server_fd; // [FIX] typo "mas_sd" -> "max_sd"

        // masukkan semua client aktif ke set
        for (int i = 0; i < MAX_CLIENTS; i++) { // [FIX] koma -> titik koma
            int sd = client_sockets[i];
            if (sd > 0) FD_SET(sd, &readfds);
            if (sd > max_sd) max_sd = sd; // [FIX] logika terbalik: < -> >
        }
        ...
        if (FD_ISSET(server_fd, &readfds)) {
            ...
            // bersihkan newline dari input username
